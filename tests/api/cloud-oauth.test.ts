...
test('config loader reads DROPBOX_APP_KEY/SECRET and tolerates absence', () => {
...
test('cookie names and redirect URIs are namespaced per provider', () => {
  ...
  assert.equal(refreshCookieName(DROPBOX_SPEC), 'dropbox_refresh');
...
test('refreshAccessToken throws OAuthTokenError on failure', async () => {
  ...
  await assert.rejects(() => refreshAccessToken(DROPBOX_SPEC, f, CONFIG, 'rt'), (err: unknown) => {
...
test('authCallback reports a missing refresh token', async () => {
